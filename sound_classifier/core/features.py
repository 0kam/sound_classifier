...
def get_log_mel_spec(input_waveform, rate:int=16000,
    ...
    window_length_samples = int(round(rate * stft_win_sec))
    hop_length_samples = int(round(rate * stft_hop_sec))
    ...
    if tflite:
        stft = STFTTflite(
            n_fft=fft_length, win_length=window_length_samples,
            hop_length=hop_length_samples, pad_begin=pad_begin, pad_end=pad_end
        )
        magnitude = MagnitudeTflite()
    else:
        stft = STFT(
            n_fft=fft_length, win_length=window_length_samples,
            hop_length=hop_length_samples, pad_begin=pad_begin, pad_end=pad_end
    ...
    x = tf.expand_dims(input_waveform, 2)
    x = stft(x)
    x = magnitude(x)
    x = tf.squeeze(x, axis = -1) # Squeeze the channel dimension
    x = tf.matmul(x, linear_to_mel_weight_matrix)
    x = tf.math.log(x + log_offset)
    model = Functional(inputs=input_waveform, outputs=x, name="log_mel_spectrogram")
    return model
